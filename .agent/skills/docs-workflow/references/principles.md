...
Conceptual invariants that govern all documentation workflow modes.
These principles are the evaluation rubric -- every decision about creating,
updating, consolidating, or analyzing docs should trace back to one or more
of these.
...
- **Commands**: validate existence (package.json scripts, Makefile targets, CI steps).
  Run non-destructive checks (`--help`, dry-run).
- **Paths**: confirm they exist in the repo.
- **Frontmatter**: check presence after generation.
